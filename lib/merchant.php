...
function get_merchantlist() {
  ...
  $array['id'] = $mid;
  $query = "SELECT merchantid,slot,item,faction_required,level_required,alt_currency_cost,classes_required,probability,min_expansion,max_expansion FROM merchantlist WHERE merchantid=$mid";
  $results = $mysql->query_mult_assoc($query);
  if ($results) {
        foreach ($results as $result) {
      $result['item_name'] = 'Item not in DB';
          $array['slots'][$result['slot']] = array("item"=>$result['item'], "item_name"=>$result['item_name'], "faction_required"=>$result['faction_required'], "level_required"=>$result['level_required'], "alt_currency_cost"=>$result['alt_currency_cost'], "classes_required"=>$result['classes_required'], "probability"=>$result['probability'], "min_expansion"=>$result['min_expansion'], "max_expansion"=>$result['max_expansion']);
        }
  }
  $query = "SELECT m.merchantid,m.slot,m.item,i.price,i.sellrate,m.faction_required,m.level_required,m.alt_currency_cost,m.classes_required,probability,min_expansion,max_expansion 
...
function update_merchantlist() {
  ...
  for ($i=1; $i<=$count; $i++){
    ...
    if (($slot != $_POST["newslot{$i}"]) || ($values['item'] != $_POST["item{$i}"]) || ($values['faction_required'] != $_POST["faction_required{$i}"]) || 
          ($values['level_required'] != $_POST["level_required{$i}"]) || ($values['alt_currency_cost'] != $_POST["alt_currency_cost{$i}"]) || ($values['classes_required'] != $_POST["classes_required{$i}"]) || ($values['probability'] != $_POST["probability{$i}"]) || ($values['min_expansion'] != $_POST["min_expansion{$i}"]) || ($values['max_expansion'] != $_POST["max_expansion{$i}"])) {
      if($_POST["newslot{$i}"] > -1){
        $query = "UPDATE merchantlist SET item=\"" . $_POST["item{$i}"] . "\", slot=\"" . $_POST["newslot{$i}"] . "\", faction_required=\"" . $_POST["faction_required{$i}"] . "\", level_required=\"" . $_POST["level_required{$i}"] . "\", alt_currency_cost=\"" . $_POST["alt_currency_cost{$i}"] . "\", classes_required=\"" . $_POST["classes_required{$i}"] . "\", probability=\"" . $_POST["probability{$i}"] . "\", min_expansion=\"" . $_POST["min_expansion{$i}"] . "\", max_expansion=\"" . $_POST["max_expansion{$i}"] . "\" WHERE merchantid=$mid AND slot=$slot";
...
function add_merchant_item() {
  ...
  $query = "INSERT INTO merchantlist SET merchantid=$mid, slot=$slot, item=$item, faction_required=$faction_required, level_required=$level_required, alt_currency_cost=$alt_currency_cost, classes_required=$classes_required, probability=$probability, min_expansion=$min_expansion, max_expansion=$max_expansion";
  $mysql->query_no_result($query);
}
...
function copy_merchantlist() {
  ...
  $query = "INSERT INTO merchantlist (slot,item,faction_required,level_required,alt_currency_cost,classes_required,probability, min_expansion, max_expansion) 
            SELECT slot,item,faction_required,level_required,alt_currency_cost,classes_required,probability, min_expansion, max_expansion FROM merchantlist where merchantid=$mid";
  $mysql->query_no_result($query);
